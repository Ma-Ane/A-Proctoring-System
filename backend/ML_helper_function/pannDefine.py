...
from backend.ML_helper_function.pytorch_utils import do_mixup, interpolate, pad_framewise_output
...
class Cnn14_no_specaug(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        if input.ndim == 3:
            ...
            raise RuntimeError(f"Bad input shape to spectrogram: {input.shape}")

        print("Spectrogram input shape:", input.shape)
